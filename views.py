...
from app import client, db, pengguna, admin_tabel, pertanyaan_identifikasi, aturan, pengetahuan, datakorban
import sklearn
...
def admin():
    if 'emailuser' in session:
        ...
        total_users = pengguna.count_documents({})
        total_pengetahuan = pengetahuan.count_documents({})
        total_aturan = aturan.count_documents({})
        total_kasus = pertanyaan_identifikasi.count_documents({})
...
def login():
    if request.method == 'POST':
        ...
        user = pengguna.find_one({'emailuser': username})
        ...
        admin = admin_tabel.find_one({'emailuser': username})
...
def signup():
    if request.method == 'POST':
        ...
        existing_user = pengguna.find_one({'emailuser': username})
        if existing_user:
